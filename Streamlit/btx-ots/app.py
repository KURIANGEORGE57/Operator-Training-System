...
LIMITS = {
    "dP_alarm": 0.30, "dP_trip": 0.33, "dP_esd": 0.34,
    "T_top_alarm": 100.0, "T_top_esd": 103.0,
    "xB_spec": 0.9990, "L_drum_min": 0.10, "L_drum_crit": 0.05,
}
...
def safety_logic(x_next: Dict, u_applied: Dict) -> Dict:
    ...
    if x_next["dP_col"] > LIMITS["dP_alarm"]: alarms.append("High column ΔP")
    if x_next["T_top"] > LIMITS["T_top_alarm"]: alarms.append("High overhead T")
    if x_next["xB_sd"] < LIMITS["xB_spec"]: alarms.append("Off-spec benzene purity")
    if x_next.get("L_Drum", 0.5) < LIMITS["L_drum_min"]: alarms.append("Low reflux drum level")
    ...
    # ESD
    if (x_next["dP_col"] > LIMITS["dP_esd"]) or (x_next["T_top"] > LIMITS["T_top_esd"]) or (x_next.get("L_Drum",0.5) < LIMITS["L_drum_crit"]):
        esd = True
